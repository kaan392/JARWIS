...
TOOL_DECLARATIONS = [
    ...
    {
        "name": "close_app",
        "description": "Açık bir uygulamayı kapatır. WhatsApp, Chrome, Spotify, PowerPoint, Explorer vb.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "app_name": {
    ...
    },
    {
        "name": "analyze_screen",
        "description": (
            "Ekran goruntusunu alip Gemini vision ile analiz eder. "
            "Kullanici ekranda ne oldugunu, bir hatayi, gorunen metni, butonlari veya pencere icerigini sordugunda kullan. "
            "Aktif pencereye ek olarak tum ekran veya mevcut goruntunun analiz edilmesini destekler."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {
                    "type": "STRING",
                    "description": "Kullanicinin ekranla ilgili sorusu. Ornek: 'Bu hatayi oku', 'Ekrandaki metni analiz et', 'Tum ekranı kontrol et'"
                },
                "target": {
                    "type": "STRING",
                    "description": "Analiz hedefi: active_window veya entire_screen."
                }
            },
            "required": ["query"]
        }
...
class JarvisLive:
    ...
    async def _speak_with_gemini(self, message: str):
        if not message or not self._loop or not self.session:
            return
        try:
            ...
            self.ui.write_log("SYS: Sesli yanıt gönderilemedi.")

    async def _execute_pending_confirmation(self, action: str) -> bool:
        action = str(action or "").lower()
        if action == "shutdown":
            self.ui.write_log("SYS: Kapatılıyor...")
            try:
                await self._speak_with_gemini("Tamam, kendimi kapatıyorum.")
            except Exception:
                pass
            ...
            self.ui.write_log("SYS: Sesli yanıtlar kapatılıyor...")
            try:
                await self._speak_with_gemini("Sesli yanıtları kapatıyorum.")
            except Exception:
                pass
            await self._interrupt_audio()
            ...
            if not self.ui.paused:
                self.ui.write_log("SYS: JARVIS duraklatılıyor...")
                try:
                    await self._speak_with_gemini("Duraklatılıyorum.")
                except Exception:
                    pass
                self.ui._toggle_pause()
            else:
                self.ui.write_log("SYS: JARVIS zaten duraklatılmış.")
                try:
                    await self._speak_with_gemini("Zaten duraklatıldı.")
                except Exception:
                    pass
            return True
        return False
    ...
    async def _handle_local_voice_command(self, text: str) -> bool:
        ...
        # === ONAY YANITLARI (Evet/Hayır) ===
        if self._contains_any(normalized, ("evet", "yes", "tamam", "olur", "tabii", "aynen")):
            if self._confirmation_pending:
                action = self._confirmation_pending
                self._confirmation_pending = None
                self.ui.write_log(f"SYS: {action} onaylandı.")
                return await self._execute_pending_confirmation(action)
            return False

        if self._contains_any(normalized, ("hayır", "hayir", "no", "iptal", "değil", "degil", "istemem", "istemiyorum")):
            if self._confirmation_pending:
                self._confirmation_pending = None
                self.ui.write_log("SYS: İşlem iptal edildi.")
                try:
                    await self._speak_with_gemini("Tamam, iptal ettim.")
                except Exception:
                    pass
                return True
            return False

        # === KAPATIŞ KOMUTLARı (Kendini Kapat) — ÖNCELİKLİ ===
        if self._contains_any(normalized, ("kendini kapat", "kendim kapat", "jarvis kapat", "jarwis kapat", "programı kapat", "sistemi kapat", "uygulamayı kapat", "kendimi kapat")):
            self._confirmation_pending = "shutdown"
            self.ui.write_log("SYS: Kendimi kapatmak istediğinden emin misin?")
            try:
                await self._speak_with_gemini("Kendimi kapatmak istediğinden emin misin? Evet veya hayır deyin.")
            except Exception:
                pass
            return True

        # === MIKROFON / SESE SES (Mute) — ÖNCELİKLİ ===
        if self._contains_any(normalized, ("mikrofonu sessize", "mikrofon sessize", "mikrofonu kapat", "mikrofon kapat", "mute", "sessiz", "sessize", "sustur", "ses kapat", "sesimi kes", "sesi kapat")):
            self._confirmation_pending = "mute"
            self.ui.write_log("SYS: Sesli yanıtları kapatmak istediğinden emin misin?")
            try:
                await self._speak_with_gemini("Sesli yanıtları kapatmak istediğinden emin misin? Evet veya hayır deyin.")
            except Exception:
                pass
            return True

        # === DURAKLAT / DUR (Pause) — ÖNCELİKLİ ===
        if self._contains_any(normalized, ("kendini durdur", "kendimi durdur", "pause", "duraklat", "dur", "durdur", "bekle", "askıya al", "askiya al")):
            self._confirmation_pending = "pause"
            self.ui.write_log("SYS: Duraklatılmak istediğinden emin misin?")
            try:
                await self._speak_with_gemini("Duraklatılmak istediğinden emin misin? Evet veya hayır deyin.")
            except Exception:
                pass
            return True

        # === EKRANI KÜÇÜLT ===
        if self._contains_any(normalized, ("simge", "küçült", "kucult", "minimize", "simge durumuna")):
            self.ui.write_log("SYS: Pencere simge durumuna küçültülüyor...")
            try:
                await self._speak_with_gemini("Pencere simge durumuna küçültülüyor.")
            except Exception:
                pass
            self.ui.minimize()
            return True
        ...
        # === İKİNCİ MONITÖRE ===
        if (
            self._contains_any(normalized, ("ikinci monitör", "2. monitör", "iki monitör", "ikinci ekran", "ikinci ekrana", "ikinci monitore", "ikinci ekrana"))
            or ("monitör" in normalized and "ikinci" in normalized)
            or ("ekran" in normalized and "ikinci" in normalized)
        ):
            self.ui.write_log("SYS: Pencere ikinci monitöre taşınıyor...")
            try:
                await self._speak_with_gemini("Pencere ikinci monitöre taşınıyor.")
            except Exception:
                pass
            self.ui.move_foreground_window_to_monitor(2)
            return True

        # === GENEL DURDUR KOMUTLARı (Konuşmayı durdur) ===
        if self._is_stop_command(text):
            await self._interrupt_audio()
            self.ui.write_log("SYS: Konuşma durduruldu.")
            try:
                await self._speak_with_gemini("Konuşma durduruldu.")
            except Exception:
                pass
            return True
    ...
    def _clean_transcript_text(text: str) -> tuple[str, bool]:
        ...
        replacements = {
            ...
            "an ne min": "annemin",
            "an ne": "anne",
            "tele fon numara sı": "telefon numarası",
            "telefon numara sı": "telefon numarası",
        }
        for pattern, replacement in replacements.items():
            raw = re.sub(r"\b" + re.escape(pattern) + r"\b", replacement, raw, flags=re.IGNORECASE)
        ...
        while index < len(tokens):
            ...
            if index + 1 < len(tokens):
                ...
                if token.isalpha() and next_tok.isalpha():
                    if (
                        len(token) <= 4
                        and len(next_tok) <= 5
                        and len(token) + len(next_tok) <= 11
                        and not (len(token) <= 2 and len(next_tok) <= 2)
                    ):
                        merged.append(token + next_tok)
                        index += 2
                        continue
                    if (
                        len(token) <= 7
                        and len(next_tok) <= 2
                        and len(token) + len(next_tok) <= 9
                    ):
                        merged.append(token + next_tok)
                        index += 2
                        continue
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "save_memory":
                ...
                result = r or "Uygulama kapatma isteği gönderildi."
    ...
    def _is_stop_command(text: str) -> bool:
        ...
        normalized = re.sub(r"[^a-zA-ZçğıöşüÇĞİÖŞÜ0-9]+", " ", text.lower()).strip()
        def compress_repeats(token: str) -> str:
            return re.sub(r"(.)\1+", r"\1", token)

        tokens = [compress_repeats(tok) for tok in normalized.split() if tok]
        stop_words = {
            "sus", "dur", "stop", "sessiz", "kes",
            "durdur", "duraklat", "pause", "bekle", "askiya", "askıya",
        }
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            raw_txt = sc.input_transcription.text.strip()
                            if raw_txt:
                                txt, _ = self._clean_transcript_text(raw_txt)
                                if txt:
                                    handled = await self._handle_local_voice_command(txt)
                                    if handled:
