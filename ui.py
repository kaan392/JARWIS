...
class JarvisUI:
    ...
    def _build_social_bar(self):
        ...
        name_lbl.bind("<Button-1>", _open("https://www.instagram.com/kyavuz2802"))
        ...
        if self._icon_ig:
            ...
            ig_lbl.bind("<Button-1>", _open("https://www.instagram.com/kyavuz2802"))

        if self._icon_yt:
            yt_lbl = tk.Label(bar, image=self._icon_yt, bg=C_BG, cursor="hand2")
            yt_lbl.pack(side="left", padx=4)
            yt_lbl.bind("<Button-1>", _open("https://www.youtube.com/@KağanYavuz-z1l"))
    ...
    def minimize(self):
        if self.root.state() == "iconic":
            return
        self.root.iconify()
        self.write_log("SYS: Pencere simge durumuna küçültüldü.")

    def _set_window_position(self, hwnd, left: int, top: int, width: int, height: int) -> bool:
        try:
            user32 = ctypes.windll.user32
            SWP_NOZORDER = 0x0004
            SWP_SHOWWINDOW = 0x0040
            return bool(user32.SetWindowPos(hwnd, 0, left, top, width, height, SWP_NOZORDER | SWP_SHOWWINDOW))
        except Exception:
            return False
    ...
    def move_to_monitor(self, monitor_index: int = 2):
        monitors = self._get_monitor_rects()
        if monitor_index < 1 or monitor_index > len(monitors):
            self.write_log("SYS: İkinci monitör bulunamadı.")
            return
        ...
        monitor_left, monitor_top, monitor_width, monitor_height = monitors[monitor_index - 1]
        target_w, target_h = self._normal_size
        target_w = min(target_w, monitor_width)
        target_h = min(target_h, monitor_height)

        new_left = monitor_left + max(0, (monitor_width - target_w) // 2)
        new_top = monitor_top + max(0, (monitor_height - target_h) // 2)
        self._window_geometry = f"{target_w}x{target_h}+{new_left}+{new_top}"
        self._set_window_position(self.root.winfo_id(), new_left, new_top, target_w, target_h)
        self._resize_surface(target_w, target_h)
        self.write_log("SYS: Pencere ikinci monitöre taşındı.")

    def move_foreground_window_to_monitor(self, monitor_index: int = 2):
        monitors = self._get_monitor_rects()
        if monitor_index < 1 or monitor_index > len(monitors):
            self.write_log("SYS: İkinci monitör bulunamadı.")
            return
        ...
        if not hwnd:
            self.write_log("SYS: Aktif pencere alınamadı.")
            return

        if self._fullscreen and hwnd == self.root.winfo_id():
            self._fullscreen = False
            self.root.attributes("-fullscreen", False)
        ...
        new_left = monitor_left + max(0, (monitor_width - target_w) // 2)
        new_top = monitor_top + max(0, (monitor_height - target_h) // 2)
        if not self._set_window_position(hwnd, new_left, new_top, target_w, target_h):
            self.write_log("SYS: Pencere taşınamadı.")
            return
        ...
        self.write_log("SYS: Aktif pencere ikinci monitöre taşındı.")
    ...
    def _refresh_brief_cards(self):
        try:
            weather = get_weather_summary("Konya")
