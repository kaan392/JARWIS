...
def _capture_screen(target: str) -> tuple[bool, str, str]:
    """Ekran görüntüsü al. (ok, file_path, window_title) döndürür."""
    if not HAS_MSS:
        return False, "mss kütüphanesi kurulu değil. 'pip install mss' ile kur.", ""

    target = str(target or "active_window").lower().strip()
    window_title = ""
    try:
        with mss.mss() as sct:
            if target in {"entire_screen", "screen", "desktop", "all_screens"}:
                monitor = sct.monitors[0]
                window_title = "tüm ekran"
            else:
                monitor = sct.monitors[1]
                window_title = _get_active_window_title() or "aktif pencere"
...
def _vision_prompt(query: str, window_title: str) -> str:
    ...
    target_label = "aktif pencere" if label != "tüm ekran" else "tüm ekran"
    return (
        "Sen Windows üzerinde JARVIS için ekran analizi yapan bir görüntü yorumlayıcısısın.\n"
        f"Aşağıdaki ekran görüntüsü {target_label} içindir.\n"
        f"Görüntü tanımı: {label}\n\n"
        "Görevlerin:\n"
...
def analyze_screen(query: str, target: str = "active_window") -> str:
    ...
    ok, result, window_title = _capture_screen(target)
    if not ok:
